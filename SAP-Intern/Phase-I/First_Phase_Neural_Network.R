...
df2<-df[,1:3]
train_indeks <- createDataPartition(df2$day, p = .7, list = FALSE, times = 1)
...
nnet_fit <- nnet(day ~., train, size = 3, decay = 0.1)
...
levels(train$day)<-make.names(levels(factor(train$day)))
train_y<-train$day
